...
- `ForgotPassword.jsx`: Password reset request form — sends reset email and now pre-fills the customer email when launched from onboarding/login recovery guidance.
...
- `CustomerSelfServicePanel.jsx`: Shared customer portal workspace used across all customer profile variants. Handles editable self-service profile fields, in-profile password changes, onboarding address normalization, registered machines/assets, auto-derived serviced machine summaries, latest review display, and grouped history by service category and assigned field agent.
- `CustomerAssetHistoryPage.jsx`: Customer-facing machine drill-down page opened from the asset panel’s Total Services action. Lists all services rendered on a selected machine/asset.

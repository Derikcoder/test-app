...
const FieldAgentSelfProfile = () => {
 ...
 const fetchAgentData = async () => {
  ...
  try {
   ...
   // Fetch agent details
   const agentResponse = await api.get('/agents/me', {
    headers: { Authorization: `Bearer ${user.token}` }
   });
   setAgent(agentResponse.data);

   // Fetch service calls assigned to this agent
   const callsResponse = await api.get('/service-calls/my-assigned', {
    headers: { Authorization: `Bearer ${user.token}` }
   });

   setServiceCalls(callsResponse.data);
   ...
   if (err.response?.data?.jobs) {
    ...
    setError(err.response?.data?.message || 'Failed to fetch your profile data');
   }
  } finally {
   setLoading(false);
  }
   ...
   <div className={pageShellClass}>
    <div className="max-w-[1200px] mx-auto">
     ...
     <div className="mb-4 flex flex-wrap gap-2">
      <span className="inline-flex items-center rounded-full border border-cyan-700 bg-cyan-950 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-cyan-200">
       {roleLabel}
      </span>
...
export default FieldAgentSelfProfile;

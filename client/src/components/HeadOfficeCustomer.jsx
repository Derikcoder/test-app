 ...
 * @description Full profile view for Head Office customers.
...
const HeadOfficeCustomer = () => {
 ...
 return (
  <CustomerProfileLayout
   ...
   badge={<span className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-400/30">Head Office</span>}
  meta={createBusinessProfileMeta({
   customer,
   variantLabel: 'Head Office',
   navigate,
  })}
  ...
  >
    ...
    <CustomerInfoGrid cards={infoCards} />
     ...
     {/* Note about child accounts */}
     <div className="rounded-xl border border-indigo-400/20 bg-indigo-500/10 px-5 py-4">
      <p className="text-xs text-indigo-300/80 leading-relaxed">
       <span className="font-semibold text-indigo-200">Head Office account</span> — Branch and Franchise locations linked to this account manage their own service calls and billing independently.
      </p>
     </div>
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {customer.sites.map((site) => (
         <div key={site._id} className="rounded-xl border border-white/15 bg-white/5 p-4 space-y-1.5">
          ...
          {site.contactPerson && <p className="site-contact-line">Contact: {site.contactPerson}</p>}
          {site.contactPhone  && <p className="site-contact-line">Phone: {site.contactPhone}</p>}

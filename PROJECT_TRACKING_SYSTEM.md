...
If there is no ritual handoff, assume the session may have ended in an abnormal shutdown first. Treat power loss, editor crash, and forced restarts as recovery events until proven otherwise.
...
Recovery sequence when no ritual handoff exists:

1. Read `continuity/CURRENT_STATUS.md` and identify the last explicit goal.
2. Compare that goal against current git status, recent commits, and checkpoint artifacts.
3. Classify the repo as Clean, Dirty, or Stale.
4. Resume only after the safety gate below passes.

Repo state classification:

1. Clean: repo state and last stated goal still align.
2. Dirty: uncommitted changes exist without a trusted handoff explaining them.
3. Stale: notes, commits, and working tree no longer tell the same story.

Safety gate before resuming:

1. The last explicit goal is known.
2. The current repo state matches or explains that goal.
3. Finished work, partial work, and planned-only work are clearly separated.
4. The next action is still correct.

If any gate fails:

1. Stop implementation work.
2. Reconstruct state from checkpoint artifacts and session logs.
3. Write a fresh continuity note before resuming.

Example recovery test:

1. If the last known goal was to begin phase 6 of the weightloss work and there is no ritual handoff after that point, do not assume phase 6 started.
2. Verify whether phase 6 was untouched, partially started, or completed.
3. Resume only from the last verified answer.
